...
export class KafkaService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info('📧 Email Service Kafka initialized successfully');

    } catch (error) {
      logger.error('Failed to initialize Kafka for Email Service:', error);
      throw error;
    }
  }
  ...
  async startConsumer(config: ConsumerConfig): Promise<void> {
    ...
    try {
      ...
      logger.info(`📧 Email Service consumer started for topics: ${config.topics.join(', ')}`);
  ...
  async publishEmailEvent(eventType: string, data: any): Promise<void> {
    const event = {
    ...
    await this.publishMessage('email-events', event);
  }
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      logger.info('📧 Email Service Kafka disconnected');
