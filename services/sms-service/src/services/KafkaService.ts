...
export class KafkaService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info('📱 SMS Service Kafka initialized successfully');

    } catch (error) {
      logger.error('Failed to initialize Kafka for SMS Service:', error);
      throw error;
    }
  }
  ...
  async startConsumer(config: ConsumerConfig): Promise<void> {
    ...
    try {
      ...
      logger.info(`📱 SMS Service consumer started for topics: ${config.topics.join(', ')}`);
  ...
  async publishSMSEvent(eventType: string, data: any): Promise<void> {
    const event = {
    ...
    await this.publishMessage('sms-events', event);
  }

  async publishDeliveryReport(report: any): Promise<void> {
    await this.publishMessage('sms-delivery-reports', report);
  }
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      logger.info('📱 SMS Service Kafka disconnected');
